...
// --- DATABASE FETCHING (LAHAT NG DATA NASA SQL NA) ---
// Kinukuha nito ang lahat ng products under 'Lightsticks' category
$query = "SELECT p.name, p.price, p.image
          FROM products p
          JOIN categories c ON p.category_id = c.id
          WHERE c.name = 'Lightsticks' AND p.stock > 0
          ORDER BY p.id DESC";
...
<head>
    <meta charset="UTF-8">
    <title>Sparkverse | Lightsticks</title>
    ...
    <style>
        ...
        .info h3 { font-size: 14px; color: #333; height: 45px; overflow: hidden; margin: 10px 0; text-transform: uppercase; font-weight: 700; }
        .price { color: #00B4D8; font-weight: bold; font-size: 18px; margin-bottom: 10px; }
...
<header class="navbar">
    <div class="nav-container">
        ...
        <nav class="nav-links">
            ...
            <a href="lightsticks.php" class="active">Lightsticks</a>
...
<div class="main-wrapper">
    <div class="container">
        <h1 class="page-title">Official Lightsticks</h1>
        <div class="product-grid">
            <?php if ($db_result && mysqli_num_rows($db_result) > 0): ?>
                <?php while($ls = mysqli_fetch_assoc($db_result)): ?>
                    <div class="product-card">
                        <img src="<?php echo htmlspecialchars($ls['image']); ?>" alt="Lightstick">
                        <div class="info">
                            <h3><?php echo htmlspecialchars($ls['name']); ?></h3>
                            <p class="price">₱ <?php echo number_format($ls['price'], 2); ?></p>
                            <button class="add-btn add-to-cart-ajax"
                                data-name="<?php echo htmlspecialchars($ls['name']); ?>"
                                data-price="<?php echo $ls['price']; ?>"
                                data-image="<?php echo htmlspecialchars($ls['image']); ?>">+</button>
                        </div>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
                <p>No lightsticks found in the database.</p>
            <?php endif; ?>
        </div>
    </div>
</div>
...
document.querySelectorAll('.add-to-cart-ajax').forEach(button => {
    button.addEventListener('click', function() {
        ...
        fetch('lightsticks.php', { method: 'POST', body: formData })
        .then(res => res.text())
        .then(data => {
