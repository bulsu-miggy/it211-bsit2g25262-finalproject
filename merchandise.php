...
// --- DATABASE FETCHING ---
// Kinukuha lahat ng products under 'Merchandise' category
$query = "SELECT p.* FROM products p 
          JOIN categories c ON p.category_id = c.id 
          WHERE c.name = 'Merchandise' AND p.stock > 0 
          ORDER BY p.id DESC";
...
<head>
    <meta charset="UTF-8">
    <title>Sparkverse | Merchandise</title>
...
<header class="navbar">
    <div class="nav-container">
        ...
        <nav class="nav-links">
            ...
            <a href="merchandise.php" class="active">Merchandise</a>
            ...
            <?php if (mysqli_num_rows($result) > 0): ?>
                <?php while($m = mysqli_fetch_assoc($result)): ?>
                    <div class="product-card">
                        <img src="<?php echo htmlspecialchars($m['image']); ?>" alt="Merch">
                        <div class="info">
                            <h3><?php echo htmlspecialchars($m['name']); ?></h3>
                            <p class="price">₱ <?php echo number_format($m['price'], 2); ?></p>
                            <button class="add-btn add-to-cart-ajax"
                                data-name="<?php echo htmlspecialchars($m['name']); ?>"
                                data-price="<?php echo $m['price']; ?>"
                                data-image="<?php echo htmlspecialchars($m['image']); ?>">+</button>
                        </div>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
...
document.querySelectorAll('.add-to-cart-ajax').forEach(button => {
    button.addEventListener('click', function() {
        ...
        fetch('merchandise.php', { method: 'POST', body: formData })
        .then(res => res.text())
        .then(data => {
