...
// Gamit ang JOIN para makuha lahat ng products sa ilalim ng 'Albums' category
$query = "SELECT p.name, p.price, p.image
          FROM products p
          JOIN categories c ON p.category_id = c.id
          WHERE c.name = 'Albums' AND p.stock > 0
          ORDER BY p.id DESC";
$db_result = mysqli_query($conn, $query);

if ($db_result) {
    while($row = mysqli_fetch_assoc($db_result)) {
        // Image logic: Check kung galing sa default folder o sa admin uploads
        $image_path = (strpos($row['image'], 'ALBUMS/') !== false) 
                      ? $row['image'] 
                      : 'uploads/' . $row['image'];
            ...
            <?php if (!empty($all_albums)): ?>
                <?php foreach ($all_albums as $album): ?>
                    <div class="product-card">
                        <img src="<?php echo $album['image']; ?>" alt="Album">
                        <div class="info">
                            <h3><?php echo htmlspecialchars($album['name']); ?></h3>
                            <p class="price">₱ <?php echo number_format($album['price'], 2); ?></p>
                            <button class="add-btn add-to-cart-ajax"
                                data-name="<?php echo htmlspecialchars($album['name']); ?>"
                                data-price="<?php echo $album['price']; ?>"
                                data-image="<?php echo $album['image']; ?>">+</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <p style="text-align: center; width: 100%;">No albums found in the database.</p>
            <?php endif; ?>
        </div>
    </div>
</div>
...
document.querySelectorAll('.add-to-cart-ajax').forEach(button => {
    button.addEventListener('click', function() {
        ...
        fetch('albums.php', { method: 'POST', body: formData })
        .then(res => res.text())
        .then(data => {
            if (data.trim() === 'success') {
                ...
                alert(name + " added to Sparkverse cart!");
            }
        });
    });
});
