...
<head>
    ...
    <title>LYNX - Women Collection</title>
...
<body>
  ...
  <div class="main-banner">
      <img src="images/WOMAN.png" alt="Model" class="model">

  </div>

  <main class="home container">
    <h2 style="font-family: 'Rubik Mono One'; margin-bottom: 20px;">WOMEN'S COLLECTION</h2>
    
    <div class="sub-nav" style="margin-bottom: 40px; display: flex; gap: 20px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">
        <a href="women.php" style="text-decoration: none; color: <?php echo $sub == '' ? 'black' : 'gray'; ?>; font-weight: bold;">ALL</a>
        <a href="women.php?sub=basic tops" style="text-decoration: none; color: <?php echo $sub == 'basic tops' ? 'black' : 'gray'; ?>;">BASIC TOPS</a>
        <a href="women.php?sub=skirts" style="text-decoration: none; color: <?php echo $sub == 'skirts' ? 'black' : 'gray'; ?>;">SKIRTS</a>
        <a href="women.php?sub=pants" style="text-decoration: none; color: <?php echo $sub == 'pants' ? 'black' : 'gray'; ?>;">PANTS</a>
        <a href="women.php?sub=outerwears" style="text-decoration: none; color: <?php echo $sub == 'outerwears' ? 'black' : 'gray'; ?>;">OUTERWEARS</a>
    </div>
    ...
    <div class="product-grid">
      <?php
        // DYNAMIC QUERY
        if ($sub != '') {
            $stmt = $conn->prepare("SELECT * FROM products WHERE main_category = 'Women' AND sub_category = ? ORDER BY id DESC");
            $stmt->execute([$sub]);
        } else {
            $stmt = $conn->prepare("SELECT * FROM products WHERE main_category = 'Women' ORDER BY id DESC");
            $stmt->execute();
        }
  ...
  <div id="productModal" style="display:none; position:fixed; z-index:999; left:0; top:0; width:100%; height:100%; background: rgba(0,0,0,0.8); align-items:center; justify-content:center;">
    <div style="background:white; padding:30px; border-radius:15px; max-width:600px; width:90%; position:relative; display:flex; gap:20px;">
        <span onclick="closeModal()" style="position:absolute; right:20px; top:10px; cursor:pointer; font-size:24px;">&times;</span>
        <img id="modalImg" src="" style="width:250px; height:300px; object-fit:cover; border-radius:10px;">
        <div>
            <h2 id="modalTitle" style="font-family: 'Rubik Mono One'; font-size: 1.2rem;"></h2>
            <p id="modalDesc" style="margin: 15px 0; color: #666;"></p>
            <h3 id="modalPrice" style="color: black; font-weight: bold;"></h3>
            <button class="btn-cart" id="modalAddToCart" style="margin-top:20px; width:100%; padding:10px; background:black; color:white; border:none; border-radius:25px; cursor:pointer;">ADD TO CART</button>
        </div>
    </div>
</div>
